...
* Both MongoDB & RabbitMQ needs to be accesiable from both the Control API & from all of the IoT devices - it will work perfectly even through the Internet and will tolerate disconnects with no issues, the Nebula workers will simply start processing whatever messages got sent to it's RabbitMQ queue in that time.
